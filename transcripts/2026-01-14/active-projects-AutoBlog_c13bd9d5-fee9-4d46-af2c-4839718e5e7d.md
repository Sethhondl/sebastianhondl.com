...
I experienced this firsthand last week. I needed the 5 Hz sweep with 1V amplitude and added mass. I opened `SysID1.txt`—wrong, that was 10 Hz. Opened `SysID2.txt`—closer, but no mass. Opened `SysID1_new.txt`—that was the right frequency, but 0.5V amplitude. Twenty minutes later, I'd found my file, but I'd also found my motivation to fix the naming problem permanently.
...
Here's what I wanted: files that tell you exactly what they contain, just from the name. Something like `2.0Hz_0.5V_mass_20251017_140823.txt` instead of `output.txt`. With descriptive names, I could glance at a directory listing and immediately spot the file I needed—no opening required.
...
My new filename format (`2.0Hz_0.5V_nomass_20251017_140823.txt`) runs about 40 characters, so the first fix was increasing the buffer:

```cpp
char outfileName[64];
```

The next step was asking the user for the test parameters before starting data collection. In C++, this meant adding some simple prompts:
...
A note on input validation: this lab code trusts that the operator enters sensible values. Production code would need bounds checking (no negative frequencies, amplitude within DAC range, etc.), but for a controlled lab environment where the same few people run all the tests, the simplicity was worth the trade-off.

## The Twist: Different Frequencies for Different Configurations

Here's where it got more interesting. The system behaves differently with and without the added mass. Adding mass increases the system's moment of inertia, which lowers its natural frequency—the heavier system simply can't respond as quickly to high-frequency inputs. This means different frequency ranges are relevant for each configuration. Without mass, the system can track higher frequencies. With mass, you need to focus on lower frequencies where the response is still meaningful.
...
Notice how the with-mass array tops out at 12 Hz while the no-mass array goes to 30 Hz—above 12 Hz, the mass-loaded system's response is so attenuated that the measurements become noise-dominated.
...
Once the data files are collected, a MATLAB script processes them in batch to generate Bode plots. The script uses regex to parse the filename and extract the test parameters:
...
**Naming conventions are architecture decisions.** A good filename scheme is essentially a contract between your data collection code and your analysis code. Before I modified anything in the C++ program, I checked how the MATLAB scripts were parsing filenames. That context-awareness meant the changes upstream would play nicely with the existing analysis pipeline.

A note on delimiter choices: I use underscores to separate fields (frequency, amplitude, mass condition, timestamp) and periods only for decimal values within numeric fields. The regex `(\d+\.?\d*)Hz` handles the decimal point correctly because it's looking for digits and optional decimal points before the `Hz` unit marker. The key is consistency—pick a scheme and stick to it.
...
Sometimes the most valuable code changes aren't about algorithms or performance. They're about making your future self's life easier when you're trying to figure out which test run actually had the conditions you need. And this pattern extends well beyond lab data—the same approach works for log files, data exports, backups, screenshots, anything where you'll later need to find a specific file among many similar ones.
...
## Assistant [2026-01-15T03:53:25.263Z]
